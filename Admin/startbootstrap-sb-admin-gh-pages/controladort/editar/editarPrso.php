...
// Definición de la clase ProductoHorneadoActualizador que se encarga de actualizar los productos horneados
class editarProductoProceso {
    ...
    // Método para actualizar un producto en proceso en la base de datos
    public function actualizarProducto($id, $nombre, $cantidad, $empleadoId) {
        ...
        // Consulta SQL para actualizar el producto en proceso en la base de datos
        $sql = "UPDATE producto_prcso SET nombre=?, cantidad=?, empleado_id=? WHERE id_prcso=?";
...
if (!empty($_POST['btneditar'])) {
    ...
    if (!empty($_POST["nombre"]) && !empty($_POST["cantidad"]) && !empty($_POST["empleadoo"])) {
        // Crear una instancia de editarProductoProceso y llamar al método actualizarProducto
        $actualizador = new editarProductoProceso($conexion);
...
$id = $_GET['id'];
// Ejecutar una consulta SQL para obtener los datos actuales del producto en proceso a partir de su ID
...
<!-- HTML del formulario para editar el producto en proceso -->
