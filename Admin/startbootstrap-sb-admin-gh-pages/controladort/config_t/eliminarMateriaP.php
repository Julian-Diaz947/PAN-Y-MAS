<?php
// Definición de la clase EliminarMateriasPrimas para manejar la eliminación de materias primas ustilizadas
class EliminarMateriasPrimas {
...
// Método para establecer el ID del producto a eliminar y validarlo como un número entero
    ...
    // Método para eliminar la materia prima utilizada de la base de datos
    public function eliminar() {
        try {
            ...
            // Preparar la consulta SQL para eliminar el producto horneado de la base de datos usando el ID
            $query = "DELETE FROM suministro_s WHERE id_s = ?";
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt->affected_rows > 0) {
                     // Si la materia prima fue eliminada, crear un mensaje de éxito
                    $this->mensaje = $this->crearMensaje("Materia prima eliminada correctamente", "success");
                   ...
                   // Si no se eliminó ninguna fila, crear un mensaje de advertencia
                    $this->mensaje = $this->crearMensaje("No se encontró la materia prima a eliminar", "warning");
    ...
    // Método para obtener el mensaje creado después de intentar eliminar 
    public function getMensaje() {
        return $this->mensaje;
    }
}

// uso de la clase para eliminar la materia prima:
try {
        // Verificar si el ID del producto a eliminar ha sido proporcionado en la URL (parámetro 'id')
    if (!empty($_GET['id'])) {
           // Crear una instancia de EliminarMateriasPrimas y ejecutar el proceso de eliminación
        $producto = new EliminarMateriasPrimas($conexion);
