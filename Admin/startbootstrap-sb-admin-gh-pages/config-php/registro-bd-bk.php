...
class UserRegistration {
    ...
    public function registerUser($userData) {
        ...
        if ($this->insertUser($userData, $hashedPassword)) {
            ...
            if (!$this->isTestingEnvironment) {
                $_SESSION['empleado'] = $userData['correo'];
            }
            // Muestra un mensaje de éxito y redirige
            $this->showAlert("Usuario registrado correctamente", "../index.php");
            return true;
        } else {
            // Muestra un mensaje de error y redirige
            $this->showAlert("Usuario no registrado, intente de nuevo", "../register.php");
            return false;
        }
    }
    ...
    private function insertUser($userData, $hashedPassword) {
        $query = $this->db->prepare("INSERT INTO empleado (nombres_apellidos, ndocumento, correo, 
        contrasena) VALUES (?, ?, ?, ?)");
        $query->bind_param("ssss", $userData['nombres_apellidos'], $userData['ndocumento'],
         $userData['correo'], $hashedPassword);
